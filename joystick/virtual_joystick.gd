...
func _update_joystick(touch_position: Vector2) -> void:
	...
	if vector.length_squared() > deadzone_size * deadzone_size:
		...
		_output_non_normalized = Vector2.ZERO
	else:
		_pressed = false
		_output = Vector2.ZERO
		_output_non_normalized = vector.normalized()
	
	if use_input_actions:
		_update_input_actions()

func _update_input_actions():
	if _output_non_normalized.x < 0:
		Input.action_press(action_left, -_output_non_normalized.x)
	elif Input.is_action_pressed(action_left):
		Input.action_release(action_left)
	if _output_non_normalized.x > 0:
		Input.action_press(action_right, _output_non_normalized.x)
	elif Input.is_action_pressed(action_right):
		Input.action_release(action_right)
	if _output_non_normalized.y < 0:
		Input.action_press(action_up, -_output_non_normalized.y)
	elif Input.is_action_pressed(action_up):
		Input.action_release(action_up)
	if _output_non_normalized.y > 0:
		Input.action_press(action_down, _output_non_normalized.y)
